...
Launcher scripts (\texttt{shadow\_boxing.bat} dan \texttt{shadow\_boxing.sh}) secara otomatis memeriksa dan menginstall dependencies sebelum menjalankan game, memastikan seamless user experience.
...
|   |-- round_manager.py       # Round system
|   |-- damage_system.py       # Damage calculation
|-- ui/                        # User interface
...
\subsubsection{Instalasi Menggunakan Launcher Scripts}
...
\begin{enumerate}
    \item Memeriksa apakah Python terinstall
    \item Memeriksa ketersediaan library yang diperlukan (opencv, mediapipe, pygame)
    \item Menginstall library yang belum tersedia secara otomatis
    \item Menjalankan game setelah semua dependencies terpenuhi
\end{enumerate}
